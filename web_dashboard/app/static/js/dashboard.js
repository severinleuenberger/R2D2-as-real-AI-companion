...
function updateCriticalStatusHeader() {
    // Update header recognition status
    const headerStatusDot = document.getElementById('status-color-dot');
    const headerStatusState = document.getElementById('header-status-state');
    const headerPerson = document.getElementById('header-person');
    const headerConfidence = document.getElementById('header-confidence');
    
    if (currentStatus && audioServiceRunning) {
        if (headerStatusDot) {
            headerStatusDot.className = `status-color-dot ${currentStatus.status}`;
        }
        if (headerStatusState) {
            headerStatusState.textContent = currentStatus.status.toUpperCase();
        }
        if (headerPerson) {
            headerPerson.textContent = currentStatus.person_identity || 'no_person';
        }
        if (headerConfidence) {
            const conf = currentStatus.confidence ? Math.round(currentStatus.confidence * 100) : 0;
            headerConfidence.textContent = `${conf}%`;
        }
    } else {
        if (headerStatusDot) headerStatusDot.className = 'status-color-dot gray';
        if (headerStatusState) headerStatusState.textContent = 'IDLE';
        if (headerPerson) headerPerson.textContent = '--';
        if (headerConfidence) headerConfidence.textContent = '--%';
    }
    ...
    if (ros && ros.isConnected) {
        if (headerConnectionDot) headerConnectionDot.className = 'connection-dot connected';
        if (headerConnectionText) headerConnectionText.textContent = 'Connected';
    } else {
        if (headerConnectionDot) headerConnectionDot.className = 'connection-dot disconnected';
        if (headerConnectionText) headerConnectionText.textContent = 'Disconnected';
    }
    
    // Update header health status
    const headerHealthIcon = document.getElementById('health-icon');
    const headerHealthText = document.getElementById('health-text');
    const headerHealthDetail = document.getElementById('health-detail');
    
    if (lastHeartbeatTime && (Date.now() - lastHeartbeatTime) < 3000) {
        if (headerHealthIcon) headerHealthIcon.textContent = '✓';
        if (headerHealthText) headerHealthText.textContent = 'System OK';
    } else {
        if (headerHealthIcon) headerHealthIcon.textContent = '✗';
        if (headerHealthText) headerHealthText.textContent = 'No Response';
    }
}
...
async function checkServiceMode() {
    try {
        const wakeApiUrl = `http://${window.location.hostname}:8079/api/status`;
        const response = await fetch(wakeApiUrl, { method: 'GET', mode: 'cors' });
        const data = await response.json();
        
        const serviceModeIndicator = document.getElementById('status-service-mode');
        if (data.service_mode_active && serviceModeIndicator) {
            serviceModeIndicator.style.display = 'flex';
        }
    } catch (e) {
        console.log('Wake API not reachable - normal mode assumed');
    }
}

// Activity tracking for auto-shutdown
let lastActivityTime = Date.now();
let inactivityTimeout = 30 * 60 * 1000; // 30 minutes
...
function startActivityTracking() {
    ...
    inactivityCheckInterval = setInterval(async () => {
        ...
        if (timeSinceActivity >= inactivityTimeout) {
            const willContinue = confirm(
                'You have been inactive for 30 minutes. The Web UI will shut down to save resources. Click OK to continue using, or Cancel to shut down.'
            );
            
            if (willContinue) {
                trackActivity(); // Reset activity
            } else {
                await exitServiceMode();
            }
        }
    }, 60000); // Check every minute
}

// Helper functions for events view
function clearEvents() {
    const container = document.getElementById('status-stream-container');
    if (container) {
        container.innerHTML = '<div class="status-stream-placeholder">Events cleared. Waiting for new updates...</div>';
    }
}
...
// Stub functions for diagnostic/topic monitoring (from diagnostics.js)
function startTopicMonitor(topic, messageType) {
    console.log(`Topic monitor requested: ${topic} (${messageType})`);
    alert(`Topic monitoring for ${topic} - Feature coming soon`);
}

function stopTopicMonitor() {
    console.log('Stop topic monitor requested');
}

function runTest(testName) {
    console.log(`Diagnostic test requested: ${testName}`);
    alert(`Diagnostic test: ${testName} - Feature coming soon`);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Start activity tracking for auto-shutdown
    ...
    if (windowInstanceManager.isActive) {
        ...
        // Start periodic header status updates
        setInterval(updateCriticalStatusHeader, 1000);
    }
});
