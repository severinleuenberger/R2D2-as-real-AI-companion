#!/usr/bin/env python3
"""R2D2 Intelligent Speech Node - ROS2 Lifecycle Node for intelligent AI assistant"""
...
class IntelligentSpeechNode(LifecycleNode):
    """ROS2 lifecycle node for intelligent speech system"""
    
    def __init__(self):
        super().__init__('intelligent_speech_node')
        self.get_logger().info("R2D2 Intelligent Speech Node initializing...")
        
        # Declare parameters
        self.declare_parameter('openai_api_key', '')
        self.declare_parameter('realtime_model', 'gpt-4o-realtime-preview-2024-12-17')
        self.declare_parameter('intelligent_realtime_voice', 'nova')
        ...
        self.declare_parameter('intelligent_instructions', 'You are an intelligent AI assistant.')
        ...
        self.get_logger().info("Intelligent Speech Node initialized")
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        try:
            ...
            self.status_publisher = ROS2StatusPublisher(self, topic_prefix='/r2d2/speech/intelligent')
            self.vad_publisher = ROS2VADPublisher(self, topic_prefix='/r2d2/speech/intelligent')
            
            self.command_sub = self.create_subscription(
                String, '/r2d2/speech/intelligent/commands', self._command_callback, 10)
            self.prompt_sub = self.create_subscription(
                String, '/r2d2/speech/intelligent/assistant_prompt', self._prompt_callback, 10)
            ...
            self.start_session_srv = self.create_service(
                Trigger, '/r2d2/speech/intelligent/start_session', self._start_session_callback)
            self.stop_session_srv = self.create_service(
                Trigger, '/r2d2/speech/intelligent/stop_session', self._stop_session_callback)
    ...
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        ...
        try:
            ...
            if auto_start:
                ...
                self.get_logger().info("Auto-start disabled (waiting for gesture trigger)")
    ...
    async def _establish_connection(self) -> bool:
        ...
        try:
            ...
            self.session_id = f"intelligent-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            voice = self.get_parameter('intelligent_realtime_voice').value
            create_session(
                self.config['db_path'], self.session_id,
                metadata={'model': self.config['realtime_model'], 'voice': voice, 'source': 'intelligent', 'mode': 'intelligent'})
            ...
            instructions = self.get_parameter('intelligent_instructions').value
            await self.client.create_session(instructions=instructions, temperature=0.8)
            self.get_logger().info("✓ Session configured (Intelligent Mode)")
            ...
            self.ros2_transcript_handler = ROS2TranscriptHandler(self.transcript_handler, self, topic_prefix='/r2d2/speech/intelligent')
            ...
            self.get_logger().info("✓ Persistent connection established (Intelligent Mode)")
            return True
    ...
    async def _start_streaming(self) -> bool:
        ...
        try:
            ...
            self.get_logger().info("✓ Streaming started (Intelligent Mode)")
    ...
    async def _stop_streaming(self) -> None:
        ...
        try:
            ...
            self.get_logger().info("✓ Streaming stopped (Intelligent Mode)")
