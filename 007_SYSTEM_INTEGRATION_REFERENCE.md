...
stateDiagram-v2
    ...
    SPEAKING --> SPEAKING: person_status = RED<br/>Reset consecutive timer
    SPEAKING --> SPEAKING: person_status = non-RED<br/>Count consecutive time<br/>(< 35s)
    SPEAKING --> IDLE: Fist gesture<br/>Immediate stop
    SPEAKING --> IDLE: Consecutive non-RED ≥ 35s<br/>Auto-stop timeout
    
    note right of IDLE
        No conversation active
        Gestures: start enabled (if RED)
        Protection: None
        Watchdog: Monitoring only
    end note
    
    note right of SPEAKING
        Conversation protected
        Timer: 35s consecutive non-RED
        Resets: Every RED seen
        Fist: Immediate override
        Publishes: person_status monitored
    end note
```
